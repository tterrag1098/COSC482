...
void Star::LoadDataToGraphicsCard()
{
    ...
    for (int i = 0; i < points; i++, idx++)
    {
        ...
        verts[idx].x = center.x + radius * std::cos(angle);
        verts[idx].y = center.y + radius * std::sin(angle);
    ...
    for (int i = 0; i < points; i++, idx++)
    {
        ...
        verts[idx].x = center.x + inner_radius * std::cos(half_angle);
        verts[idx].y = center.y + inner_radius * std::sin(half_angle);
        if (color) colors[idx] = *color;
        else colors[idx] = {rand_float(), rand_float(), rand_float()};
    }
