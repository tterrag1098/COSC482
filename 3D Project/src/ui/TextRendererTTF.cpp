...
TextRendererTTF::TextRendererTTF(std::string fontFile)
{
    ...
    shader.load(vertShade.c_str(), fragShade.c_str());
    ...
    rotmat = glGetUniformLocation(shader.program, "rotMat");
...
GLboolean TextRendererTTF::isError() const
{
...
GLuint TextRendererTTF::getFontSize() const
{
    return fontSize;
}
...
\param col --- Vector of 4 floats representing (r, g, b, a) values.

*/
void TextRendererTTF::setColor(glm::vec4 col)
{
...
void TextRendererTTF::draw(std::string text, GLuint xpos, GLuint ypos, GLfloat rot) const
{
...
void TextRendererTTF::draw(const char* text, GLuint xpos, GLuint ypos, GLfloat rot) const
{
    if (isError())
        return;
...
int TextRendererTTF::textWidth(std::string text) const
{
